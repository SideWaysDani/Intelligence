...
@dag(default_args=default_args, schedule_interval=None, start_date=days_ago(2), tags=['leads_india_1_generate dag'])
def leads_india_1_generate_taskflow_api():
    ...
    def lead_dag():
      ...
      def fetch_all_companiesFortune1000India():
          ...
          sql="""SELECT ticker FROM stocktrader.fortune_india_500;""" 
          try:
              # read database configuration
              params = config()
              # connect to the PostgreSQL database
      ...
      class LeadsMaverickHelper :
          ...
          def insert_lead(self, stock_name, lead_date):
              try:
                  ...
                  cur.execute(
                      "SELECT id FROM stocktrader.leads_india_1 WHERE stock_name = %s AND lead_date >= %s AND lead_date <= %s",
                  ...
                  if existing_record:
                      ...
                      insert_sql = "INSERT INTO stocktrader.leads_india_1 (stock_name, lead_date) VALUES (%s, %s)"
      ...
      class EodhdHelper:
          ...
          def get_eod_data(self, symbol, start_date=None, end_date=None):
              url = f"{self.base_url}/eod/{symbol}"
              params = {
                "api_token": self.api_key,
                "from": start_date,  # Optional start date in 'YYYY-MM-DD' format
                "to": end_date  # Optional end date in 'YYYY-MM-DD' format
            }
            ...
            # Check for response status code
              if response.status_code == 200:
                # Parse the response as CSV
                  ...
                  print(f"Error fetching EOD data for {symbol}: {response.status_code} - {response.text}")
                  return None

          def get_indian_data(self, symbol, start_date=None, end_date=None):
              """
              Fetches end-of-day (EOD) data for a given indian stock symbol.

              Parameters:
              - symbol: The indian stock symbol (e.g., 'RELIANCE.NSE').
              - start_date: (Optional) The start date for the EOD data in 'YYYY-MM-DD' format.
              - end_date: (Optional) The end date for the EOD data in 'YYYY-MM-DD' format.

              Returns:
              - A pandas DataFrame containing the indian stock EOD data.
              """
              # Define the API endpoint for fetching crypto EOD data
              url = f"{self.base_url}/eod/{symbol}"
              params = {
                  "api_token": self.api_key,
                  "from": start_date,  # Optional start date
                  "to": end_date,  # Optional end date
                  "fmt": "csv"  # Ensure the response is in CSV format
              }

              # Make the GET request
              response = requests.get(url, params=params)

              # Debug: Print the response status code and part of the response text
              print(f"Response Status Code: {response.status_code}")
              print(f"Response Text: {response.text[:200]}")  # Show a preview of the response

              # Check if the response status is OK (200)
              if response.status_code == 200:
                  try:
                      # Parse the CSV response and convert it into a Pandas DataFrame
                      crypto_data = pd.read_csv(StringIO(response.text))
                      return crypto_data
                  except Exception as e:
                      print(f"Error parsing CSV for {symbol}: {e}")
                      return None
              else:
                  print(f"Error fetching indian stock data for {symbol}: {response.status_code} - {response.text}")
                  return None
      ...
      def temp_load_data_from_api(df_json):
          url = "https://stapi03.azurewebsites.net/api/stapi03"
          params = {
            "code": "KAgP7r6c963rwBttul9Omjr22E1tZ_UGPa0KjwFyvLzlAzFuFx_PMw=="
        }
          headers = {"Content-Type": "application/json"}
        #response = requests.get(url, params=params)

          response = requests.post(url, data=df_json, headers=headers, params=params)
      ...
      # Fetch and process stock data for a symbol
      def load_data_from_api(st_name, end_date=datetime.datetime.now().strftime("%Y-%m-%d")):
          start_date="2020-01-01"
          api_key = "610e336dc007a9.41502790"
        # Create an instance of the helper
          eodhd_helper = EodhdHelper(api_key)
          stock_symbol = st_name  # Define the stock symbol
          start_date = start_date  # Optional: start date (YYYY-MM-DD)
          end_date = end_date  # Optional: end date (YYYY-MM-DD)

        # Fetch the historical data
      #     price_data = eodhd_helper.get_eod_data(stock_symbol, start_date, end_date)
          price_data = eodhd_helper.get_indian_data(stock_symbol, start_date, end_date)

          if price_data is not None and not price_data.empty:
            # Add the 'Date' column based on timestamp (if applicable)
              if 'timestamp' in price_data.columns:
                  price_data['Date'] = pd.to_datetime(price_data['timestamp'] * 1000000)  # Convert timestamp to DateTime
              elif 'Date' in price_data.columns:
                  price_data['Date'] = pd.to_datetime(price_data['Date'])  # If the 'Date' column already exists

            # Add stock symbol to the DataFrame
            # price_data['Symbol'] = stock_symbol

            # print(price_data.head())  # Display first few rows of the data
          json_data = price_data.to_json(orient='records')
          data=temp_load_data_from_api(json_data)
          return data


      def generate_leadsFortune1000(end_date):
      # Fetch all companies
          companies = fetch_all_companiesFortune1000India()
      # print("hello")
      ...
      en = datetime.datetime(2025, 3, 22)
...
leads_phases_script_dag = leads_india_1_generate_taskflow_api()
